import { c as createLucideIcon, ap as commonjsGlobal, aq as getDefaultExportFromCjs, r as reactExports, ar as clsx, as as React, j as jsxRuntimeExports, g as cn, l as useStore, B as Button, m as Plus, C as Card, i as CardHeader, k as CardTitle, A as TrendingUp, n as CardContent, at as ChartNoAxesColumn, ab as Package, L as Label$1, I as Input, au as Banknote, av as Smartphone, a1 as CreditCard, K as FileText, v as Badge, T as Table, o as TableHeader, p as TableRow, q as TableHead, s as TableBody, t as TableCell } from "./index-Bt77HP0S.js";
import { T as Tabs, a as TabsList, b as TabsTrigger, d as TabsContent } from "./tabs-FRlTL3i_.js";
...
function fmt(n2) {
  ...
  }).format(Math.round(n2));
}
function fmtShort(n2) {
  if (Math.abs(n2) >= 1e5) return `₹${(n2 / 1e5).toFixed(1)}L`;
...
function ReportsPageInner({
  ...
  const monthlyProfitData = reactExports.useMemo(() => {
    const now2 = /* @__PURE__ */ new Date();
    const profitByKey = {};
    for (const inv of invoices) {
      const invKey = inv.date.slice(0, 7);
      const cost = inv.items.reduce(
        (c2, item) => c2 + item.purchaseCost * item.quantity,
        0
      );
      profitByKey[invKey] = (profitByKey[invKey] ?? 0) + (inv.totalAmount - cost);
    }
    const months = [];
    for (let i = 11; i >= 0; i--) {
      const d2 = new Date(now2.getFullYear(), now2.getMonth() - i, 1);
      const key = `${d2.getFullYear()}-${String(d2.getMonth() + 1).padStart(2, "0")}`;
      const month = d2.toLocaleDateString("en-IN", { month: "short" });
      const label = d2.toLocaleDateString("en-IN", {
        month: "short",
        year: "numeric"
      });
  ...
  const total12MonthProfit = reactExports.useMemo(
    () => monthlyProfitData.reduce((s2, d2) => s2 + d2.profit, 0),
    [monthlyProfitData]
  );
  const [dailyDate, setDailyDate] = reactExports.useState(getToday());
  const [profitFrom, setProfitFrom] = reactExports.useState(() => {
    const d2 = /* @__PURE__ */ new Date();
    d2.setDate(d2.getDate() - 7);
    return d2.toISOString().slice(0, 10);
  });
  const [profitTo, setProfitTo] = reactExports.useState(getToday());
  const dailyInvoices = reactExports.useMemo(
    () => invoices.filter((inv) => inv.date.slice(0, 10) === dailyDate),
    [invoices, dailyDate]
  );
  const dailySummary = reactExports.useMemo(() => {
    const totalSales2 = dailyInvoices.reduce((s2, inv) => s2 + inv.totalAmount, 0);
    const totalCashReceived2 = dailyInvoices.filter((inv) => inv.paymentMode === "cash").reduce((s2, inv) => s2 + inv.paidAmount, 0);
    const totalUpiReceived2 = dailyInvoices.filter((inv) => inv.paymentMode === "upi").reduce((s2, inv) => s2 + inv.paidAmount, 0);
    const totalOnlineReceived2 = dailyInvoices.filter((inv) => inv.paymentMode === "online").reduce((s2, inv) => s2 + inv.paidAmount, 0);
    const totalCreditSales2 = dailyInvoices.filter((inv) => inv.paymentMode === "credit").reduce((s2, inv) => s2 + inv.totalAmount, 0);
    const totalDueAmount2 = dailyInvoices.reduce(
      (s2, inv) => s2 + (inv.dueAmount ?? 0),
      0
    );
    const creditInvoices2 = dailyInvoices.filter(
      (inv) => inv.paymentMode === "credit"
    );
    const totalCollection2 = totalCashReceived2 + totalUpiReceived2 + totalOnlineReceived2;
    const dailyProfit2 = dailyInvoices.reduce((s2, inv) => {
      const cost = inv.items.reduce(
        (c2, item) => c2 + item.purchaseCost * item.quantity,
        0
      );
      return s2 + (inv.totalAmount - cost);
    }, 0);
    return {
      totalSales: totalSales2,
      totalCashReceived: totalCashReceived2,
      totalUpiReceived: totalUpiReceived2,
      totalOnlineReceived: totalOnlineReceived2,
      totalCreditSales: totalCreditSales2,
      totalDueAmount: totalDueAmount2,
      creditInvoices: creditInvoices2,
      totalCollection: totalCollection2,
      dailyProfit: dailyProfit2
    };
  }, [dailyInvoices]);
  const dailyItemRows = reactExports.useMemo(() => {
    const itemMap = {};
  ...
  const profitData = reactExports.useMemo(() => {
    const profitInvoices = invoices.filter((inv) => {
      const d2 = inv.date.slice(0, 10);
      return d2 >= profitFrom && d2 <= profitTo;
    });
    ...
    for (const inv of profitInvoices) {
      for (const item of inv.items) {
        ...
        productProfitMap[item.productId].revenue += item.quantity * item.sellingRate;
        productProfitMap[item.productId].cost += item.purchaseCost * item.quantity;
      }
    }
    const profitRows2 = Object.values(productProfitMap).sort(
      (a2, b2) => b2.revenue - b2.cost - (a2.revenue - a2.cost)
    );
    const totalRevenue2 = profitRows2.reduce((s2, r) => s2 + r.revenue, 0);
    const totalCost2 = profitRows2.reduce((s2, r) => s2 + r.cost, 0);
    const totalProfitAll2 = totalRevenue2 - totalCost2;
    const avgMarginPct2 = totalCost2 > 0 ? totalProfitAll2 / totalCost2 * 100 : 0;
    return {
      profitRows: profitRows2,
      totalRevenue: totalRevenue2,
      totalCost: totalCost2,
      totalProfitAll: totalProfitAll2,
      avgMarginPct: avgMarginPct2
    };
  ...
  const { profitRows, totalRevenue, totalCost, totalProfitAll, avgMarginPct } = profitData;
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-col gap-6", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 md:px-6 pb-6 flex flex-col gap-4", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between gap-3 flex-wrap", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-xl font-bold", children: "Reports" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground text-sm", children: "Sales, stock, and profit analytics" })
      ] }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      Card,
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(CardHeader, { className: "pb-2 pt-4 px-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between gap-3 flex-wrap", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs(CardTitle, { className: "text-sm flex items-center gap-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(TrendingUp, { size: 15, className: "text-emerald-600" }),
                "12-Month Profit Trend"
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground mt-0.5", children: "Last 12 months performance" })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-right", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: "Total Profit" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  className: `text-base font-bold ${total12MonthProfit >= 0 ? "text-emerald-600" : "text-red-600"}`,
                  children: fmt(total12MonthProfit)
                }
              )
            ] })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "px-2 pb-3", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
            ChartContainer,
            {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("defs", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      "linearGradient",
                      {
                        ...
                        y2: "1",
                        children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx("stop", { offset: "5%", stopColor: "#16A34A", stopOpacity: 0.18 }),
                          /* @__PURE__ */ jsxRuntimeExports.jsx("stop", { offset: "95%", stopColor: "#16A34A", stopOpacity: 0.02 })
                        ]
                      }
                    ) }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                        ...
                        content: /* @__PURE__ */ jsxRuntimeExports.jsx(
                          ChartTooltipContent,
                          {
                            ...
                            labelFormatter: (_, payload) => {
                              var _a, _b;
                              return ((_b = (_a = payload == null ? void 0 : payload[0]) == null ? void 0 : _a.payload) == null ? void 0 : _b.label) ?? "";
                            }
                          }
                        )
                      }
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(Tabs, { defaultValue: "daily", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsList, { "data-ocid": "reports.tab", className: "mb-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsTrigger, { value: "daily", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(ChartNoAxesColumn, { size: 14, className: "mr-2" }),
          "Daily Report"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsTrigger, { value: "stock", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Package, { size: 14, className: "mr-2" }),
          "Stock Report"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs(TabsTrigger, { value: "profit", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(TrendingUp, { size: 14, className: "mr-2" }),
          "Profit Report"
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(TabsContent, { value: "daily", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-5", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "shadow-sm border-border", children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "p-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-wrap items-center gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              Button,
              {
                size: "sm",
                variant: dailyDate === getToday() ? "default" : "outline",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              ...
              {
                size: "sm",
                variant: dailyDate === getYesterday() ? "default" : "outline",
                className: "text-xs h-8",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { className: "text-xs text-muted-foreground whitespace-nowrap", children: "Custom Date:" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              Input,
              {
                ...
                type: "date",
                value: dailyDate,
                onChange: (e3) => setDailyDate(e3.target.value),
                className: "w-36 h-8 text-xs"
              }
            )
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "ml-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm font-semibold text-muted-foreground", children: formatDateLabel(dailyDate) }) })
        ] }) }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 sm:grid-cols-4 gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "shadow-sm border-blue-200 bg-blue-50/40", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CardContent, { className: "p-3 flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-9 h-9 rounded-lg bg-blue-100 flex items-center justify-center flex-shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(IndianRupee, { size: 16, className: "text-blue-600" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: "Total Sales" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-blue-700 text-sm", children: fmt(totalSales) })
            ] })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "shadow-sm border-green-200 bg-green-50/40", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CardContent, { className: "p-3 flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-9 h-9 rounded-lg bg-green-100 flex items-center justify-center flex-shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Banknote, { size: 16, className: "text-green-600" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: "Cash" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-green-700 text-sm", children: fmt(totalCashReceived) })
            ] })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "shadow-sm border-purple-200 bg-purple-50/40", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CardContent, { className: "p-3 flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-9 h-9 rounded-lg bg-purple-100 flex items-center justify-center flex-shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Smartphone, { size: 16, className: "text-purple-600" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: "UPI" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-purple-700 text-sm", children: fmt(totalUpiReceived) })
            ] })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "shadow-sm border-cyan-200 bg-cyan-50/40", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CardContent, { className: "p-3 flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-9 h-9 rounded-lg bg-cyan-100 flex items-center justify-center flex-shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Wifi, { size: 16, className: "text-cyan-600" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: "Online" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-cyan-700 text-sm", children: fmt(totalOnlineReceived) })
            ] })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "shadow-sm border-emerald-300 bg-emerald-50/50", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CardContent, { className: "p-3 flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-9 h-9 rounded-lg bg-emerald-100 flex items-center justify-center flex-shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Banknote, { size: 16, className: "text-emerald-700" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: "Total Collection" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-emerald-700 text-sm", children: fmt(totalCollection) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-[10px] text-muted-foreground", children: "Cash + UPI + Online" })
            ] })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "shadow-sm border-orange-200 bg-orange-50/40", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CardContent, { className: "p-3 flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-9 h-9 rounded-lg bg-orange-100 flex items-center justify-center flex-shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(CreditCard, { size: 16, className: "text-orange-600" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: "Credit Sales" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-orange-700 text-sm", children: fmt(totalCreditSales) })
            ] })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "shadow-sm border-red-200 bg-red-50/40", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CardContent, { className: "p-3 flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-9 h-9 rounded-lg bg-red-100 flex items-center justify-center flex-shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(IndianRupee, { size: 16, className: "text-red-600" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: "Total Due" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-red-600 text-sm", children: fmt(totalDueAmount) })
            ] })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "shadow-sm border-border", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CardContent, { className: "p-3 flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-9 h-9 rounded-lg bg-secondary flex items-center justify-center flex-shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(FileText, { size: 16, className: "text-muted-foreground" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: "Total Invoices" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-sm", children: dailyInvoices.length }),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            Card,
            {
              className: `shadow-sm ${dailyProfit >= 0 ? "border-emerald-200 bg-emerald-50/40" : "border-red-200 bg-red-50/40"}`,
              children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CardContent, { className: "p-3 flex items-center gap-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "div",
                  {
                    className: `w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${dailyProfit >= 0 ? "bg-emerald-100" : "bg-red-100"}`,
                    children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                      TrendingUp,
                      {
                        size: 16,
                        className: dailyProfit >= 0 ? "text-emerald-600" : "text-red-600"
                      }
                    )
                  }
                ),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "div",
                    {
                      className: `font-bold text-sm ${dailyProfit >= 0 ? "text-emerald-700" : "text-red-600"}`,
                      children: fmt(dailyProfit)
                    }
                  )
                ] })
              ] })
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "shadow-card border-border", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(CardHeader, { className: "pb-2 pt-4 px-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CardTitle, { className: "text-sm flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(FileText, { size: 15, className: "text-muted-foreground" }),
            "Invoice Breakdown",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "p-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-x-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Table, { children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs(TableBody, { children: [
              dailyInvoices.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(TableRow, { "data-ocid": "reports.daily.empty_state", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                TableCell,
                {
                  colSpan: 8,
                  className: "text-center py-8 text-muted-foreground text-sm",
                  children: dailyDate === getToday() ? "No sales today yet" : "No sales on this date"
                }
              ) }),
              dailyInvoices.map((inv, idx) => {
                ...
                return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  TableRow,
                  {
                    ...
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm font-medium", children: inv.invoiceNumber }),
                      /* @__PURE__ */ jsxRuntimeExports.jsxs(TableCell, { className: "text-sm", children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: inv.customerName }),
                        inv.customerMobile && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: inv.customerMobile })
                      ] }),
                      /* @__PURE__ */ jsxRuntimeExports.jsxs(TableCell, { className: "text-sm", children: [
                        inv.items.length,
                        " items"
                      ] }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm font-medium", children: fmt(inv.totalAmount) }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm text-green-700", children: fmt(inv.paidAmount) }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm", children: due > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-bold text-red-600", children: fmt(due) }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-green-600 text-xs", children: "✓ Clear" }) }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                        Badge,
                        {
                          variant: "outline",
                          className: `text-xs capitalize ${inv.paymentMode === "credit" ? "bg-orange-50 text-orange-700 border-orange-300" : inv.paymentMode === "upi" ? "bg-purple-50 text-purple-700 border-purple-300" : inv.paymentMode === "cash" ? "bg-green-50 text-green-700 border-green-300" : "bg-cyan-50 text-cyan-700 border-cyan-300"}`,
        ...
        dailyItemRows.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "shadow-sm border-border", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(CardHeader, { className: "pb-2 pt-4 px-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CardTitle, { className: "text-sm flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(TrendingUp, { size: 15, className: "text-emerald-600" }),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "p-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-x-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Table, { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHeader, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(TableRow, { className: "bg-secondary/50", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Qty Sold" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Revenue" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Cost" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Profit" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Margin %" })
            ] }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableBody, { children: dailyItemRows.map((row) => {
              const profit = row.revenue - row.cost;
              const pct = row.cost > 0 ? profit / row.cost * 100 : 0;
              return /* @__PURE__ */ jsxRuntimeExports.jsxs(TableRow, { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm font-medium", children: row.name }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm", children: row.qty }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm", children: fmt(row.revenue) }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm text-muted-foreground", children: fmt(row.cost) }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  TableCell,
                  {
                    className: `text-sm font-semibold ${profit >= 0 ? "text-emerald-700" : "text-red-600"}`,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(TabsContent, { value: "stock", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "shadow-card border-border", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(CardHeader, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardTitle, { className: "text-base", children: "Current Stock Status" }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "p-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-x-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Table, { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(TableHeader, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(TableRow, { className: "bg-secondary/50", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Product" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Stock" }),
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Batches" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Status" })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(TableBody, { children: products.map((p2, idx) => {
            const stock = getProductStock(p2.id);
            const value = getStockValue(p2.id);
            const batchCount = getProductBatches(p2.id).length;
            const isLow = stock < p2.minStockAlert;
            return /* @__PURE__ */ jsxRuntimeExports.jsxs(
              TableRow,
              {
                "data-ocid": `reports.stock.item.${idx + 1}`,
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm font-medium", children: p2.name }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs(TableCell, { className: "text-sm", children: [
                    stock,
                    " ",
                    p2.unit
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm", children: fmt(value) }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm", children: fmt(p2.sellingPrice) }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm", children: batchCount }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { children: isLow ? /* @__PURE__ */ jsxRuntimeExports.jsx(
                    Badge,
                    {
                      variant: "destructive",
                      className: "text-xs",
                  ...
                  ) : stock === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx(Badge, { className: "text-xs bg-secondary text-muted-foreground border-0", children: "Out of Stock" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Badge, { className: "text-xs bg-success-light text-success border-0", children: "OK" }) })
                ]
              },
              p2.id
            );
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(TabsContent, { value: "profit", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-wrap items-end gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { className: "text-sm", children: "From Date" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              Input,
              {
                ...
                type: "date",
                value: profitFrom,
                onChange: (e3) => setProfitFrom(e3.target.value),
                className: "w-40 mt-1"
              }
            )
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Label$1, { className: "text-sm", children: "To Date" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              Input,
              {
                ...
                type: "date",
                value: profitTo,
                onChange: (e3) => setProfitTo(e3.target.value),
                className: "w-40 mt-1"
              }
            )
          ] })
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-blue-50 rounded-lg p-3 text-center", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground mb-1", children: "Total Revenue" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-brand-blue text-lg", children: fmt(totalRevenue) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-secondary/60 rounded-lg p-3 text-center", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground mb-1", children: "Total Cost" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-foreground text-lg", children: fmt(totalCost) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground mb-1", children: "Total Profit" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "div",
                  {
                    className: `font-bold text-lg ${totalProfitAll >= 0 ? "text-success" : "text-danger"}`,
                    children: fmt(totalProfitAll)
                  }
                )
              ]
            }
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-emerald-50 rounded-lg p-3 text-center", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground mb-1", children: "Avg Margin %" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "font-bold text-emerald-600 text-lg", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(Card, { className: "shadow-card border-border", children: /* @__PURE__ */ jsxRuntimeExports.jsx(CardContent, { className: "p-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-x-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Table, { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(TableHeader, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(TableRow, { className: "bg-secondary/50", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Product" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Qty Sold" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Cost/Unit" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Sell/Unit" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Profit/Unit" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Total Profit" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(TableHead, { className: "text-xs", children: "Margin %" })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(TableBody, { children: [
            profitRows.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(TableRow, { "data-ocid": "reports.profit.empty_state", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
              TableCell,
              {
                colSpan: 7,
                className: "text-center py-8 text-muted-foreground text-sm",
                children: "No sales data for selected range"
              }
            ) }),
            profitRows.map((row, idx) => {
              const profit = row.revenue - row.cost;
              const profitPct = row.cost > 0 ? profit / row.cost * 100 : 0;
              return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                TableRow,
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm font-medium", children: row.name }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm", children: row.qty }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm text-muted-foreground", children: row.qty > 0 ? fmt(row.cost / row.qty) : "-" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm", children: row.qty > 0 ? fmt(row.revenue / row.qty) : "-" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      TableCell,
                      {
                        className: `text-sm font-semibold ${profit / (row.qty || 1) >= 0 ? "text-success" : "text-danger"}`,
                        children: row.qty > 0 ? fmt(profit / row.qty) : "-"
                      }
                    ),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      TableCell,
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      Badge,
                      {
                        className: `text-xs border-0 ${profitPct > 0 ? "bg-success-light text-success" : "bg-danger-light text-danger"}`,
                        children: [
                          profitPct.toFixed(1),
                          "%"
                        ]
            ...
            profitRows.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(TableRow, { className: "bg-secondary/50 font-bold", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { className: "text-sm", children: "Total" }),
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                TableCell,
                {
                  className: `text-sm ${totalRevenue - totalCost >= 0 ? "text-success" : "text-danger"}`,
                  children: fmt(totalRevenue - totalCost)
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx(TableCell, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                Badge,
...
const ReportsPage = React.memo(ReportsPageInner);
export {
  ReportsPage,
